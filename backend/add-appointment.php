...
if (isset($_POST['submit'])) {
    ...
    $stmt = $connection->prepare("INSERT INTO users (name,email,contact,password,role,is_active,address,image,speciality_id) VALUES(?,?,?,?,?,?,?,?,?)");
    $stmt->bind_param("sssssssss", $name, $email, $contact, $password, $role, $status, $address,$image,$speciality);
    if ($stmt->execute()) {
...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $stmt1 = $connection->prepare("SELECT * FROM users WHERE id = ?");
    ...
    if (isset($_POST['update'])) {
        ...
        if(move_uploaded_file($tmpname,$folder)){
            $stmt2 = $connection->prepare("UPDATE users SET image = ?,name = ?,contact = ?,is_active = ?, address = ?, speciality_id = ? WHERE id = ?");
            $stmt2->bind_param("sssssss",$image, $name, $contact, $status, $address, $speciality, $get_id);
        }else{
            $stmt2 = $connection->prepare("UPDATE users SET name = ?,contact = ?,is_active = ?, address = ?, speciality_id = ? WHERE id = ?");
            $stmt2->bind_param("ssssss", $name, $contact, $status, $address, $speciality, $get_id);
        }
...
<body>
    ...
    <section class="admin">
        <div class="dashboard-wrap">
            <div class="heading">
                <h2><?php if(isset($row1)){ echo "Update"; }else{ echo "Add";  }?> Form</h2>
            </div>

...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    if (isset($_POST['update'])) {
        ...
        $stmt2 = $connection->prepare("UPDATE users SET name = ?,contact = ?,is_active = ?, address = ? WHERE id = ?");
        $stmt2->bind_param("sssss", $name, $contact, $status, $address, $get_id);
        if ($stmt2->execute()) {
...
<body>
    ...
    <section class="admin">
        <div class="dashboard-wrap">
            <div class="heading">
                <h2><?php if(isset($row1)){ echo "Update"; }else{ echo "Add";  }?> User</h2>
            </div>
